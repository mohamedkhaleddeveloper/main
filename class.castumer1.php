...
$dbh = new PDO("mysql:dbhost=localhost;dbname=alsubtai_main", "alsubtai_main", "p[;zpixEo+Si");
...
class invoices
{
    ...
    {
        global $dbh;
        $sql = $dbh->prepare("SELECT `invoice`.`idi`, `invoice`.`bill`, `invoice`.`orderid`, `ordero`.`ido`, `ordero`.`castumerid`,
    ...
    public static function retreiveTotalAllInvoices($idate , $idp)
    {
     ...
     public static function retreiveTotalAllInvoicesDate($datefrom, $dateto, $idp)
    {
        global $dbh;
        $sql = $dbh->prepare("SELECT sum(`totalm`) as `totalun` FROM `invoice` WHERE `idate` >= '$datefrom' AND `idate` <= '$dateto' AND `payment` = '$idp'");
        $sql->execute();
        $row = $sql->fetch(PDO::FETCH_ASSOC);
        $sum = $row['totalun'];
        return $sum;
    ...
    public static function  totalAllInoicesDate ($datefrom, $dateto, $payment){
        ...
        $namea = $value['namepa'];
        $namee = $value['namepe'];
        $wayOfPayment  = invoices::retreiveTotalAllInvoicesDate($datefrom, $dateto, $idp);
